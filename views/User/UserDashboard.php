...
if (isset($_POST['register_event'])) {
    ...
    if ($check->num_rows > 0) {
        ...
        $event_name = $conn->real_escape_string($event_row['name']);

        // Insert into registrations including event_name
        $sql = "INSERT INTO registrations (user_id, event_id, event_name, status) 
                VALUES ($user_id, $event_id, '$event_name', 'Pending')";
        if ($conn->query($sql)) {
...
if (isset($_POST['register_team'])) {
    ...
    if ($check->num_rows > 0) {
        ...
        // Insert team
        $conn->query("INSERT INTO teams (team_name, event_id, created_by) VALUES ('$team_name', $event_id, $user_id)");
        ...
        foreach ($players as $i => $player_name) {
            ...
            $conn->query("INSERT INTO players (team_id, player_name, student_id) VALUES ($team_id, '$pname', '$sid')");
        }

        // Register team including event_name
        $conn->query("INSERT INTO registrations (user_id, event_id, team_id, event_name, status) 
                      VALUES ($user_id, $event_id, $team_id, '$event_name', 'Pending')");
            ...
            <table class="table table-hover align-middle">
                ...
                while($reg = $history->fetch_assoc()):
                    ...
                    if($reg['team_id']) {
                        ...
                        echo "<td><strong>".htmlspecialchars($reg['team_name'])."</strong><br>";
